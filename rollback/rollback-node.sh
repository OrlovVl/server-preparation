...
if [ -f "docker-compose.yml" ]; then
    echo "[*] Останавливаем и удаляем контейнеры через $DOCKER_COMPOSE down..."
    $DOCKER_COMPOSE down
...
if [ -z "$CONTENT" ]; then
    ...
    echo "    $CONTENT"
    echo "[!] Папка не удалена, чтобы не повредить чужие данные."
    echo "[*] Вы можете удалить её вручную, если уверены."
fi
